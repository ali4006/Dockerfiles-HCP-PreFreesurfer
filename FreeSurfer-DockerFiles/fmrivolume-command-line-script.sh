...
if [ ${REPROZIP_FLAG} = true ]; then
  ...
  reprozip trace GenericfMRIVolumeProcessingPipelineBatch.sh --StudyFolder=$PWD --Subjlist=${SUBJECT_FOLDER_ORIGINAL_NUMBER} --runlocal || die ${INITDIR} "Pipeline failed."
else
  source ${FREESURFER_HOME}/SetUpFreeSurfer.sh
  GenericfMRIVolumeProcessingPipelineBatch.sh --StudyFolder=$PWD --Subjlist=${SUBJECT_FOLDER_ORIGINAL_NUMBER} --runlocal                || die ${INITDIR} "Pipeline failed."
fi
